...
class MeterActivity : AppCompatActivity() {
    private fun shouldScan(): Boolean = intent.getStringExtra("mac").isNullOrBlank()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btnScan.setOnClickListener {
            ...
            if (checkAndRequestPermissions()) if (shouldScan()) startScan()
        }
        ...
        // DIRECT CONNECT from ScanActivity (no scanning here)
        runCatching {
            val mac = intent.getStringExtra("mac")
            val name = intent.getStringExtra("name") ?: "Unknown"
            // hide scan widgets if any
            findViewById<View?>(R.id.btnScan)?.visibility = View.GONE
            findViewById<View?>(R.id.list)?.visibility = View.GONE
            findViewById<View?>(R.id.listView)?.visibility = View.GONE
            findViewById<View?>(R.id.searchBox)?.visibility = View.GONE
    ...
    override fun onRequestPermissionsResult(rc: Int, p: Array<out String>, r: IntArray) {
        ...
        if (rc == PERM_REQUEST && r.all { it == PackageManager.PERMISSION_GRANTED }) if (shouldScan()) startScan()
    ...
    private fun if (shouldScan()) startScan() {
        if (scanning) return
